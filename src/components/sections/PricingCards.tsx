  ...
  return (
    <Section bg={preview ? "gray" : "white"}>
      ...
      <div className="mb-12 flex items-center justify-center gap-3">
        <span className={`text-sm font-medium font-body ${!annual ? "text-gray-900 dark:text-white" : "text-gray-400"}`}>
          Monthly
        </span>
        <button
          onClick={() => setAnnual(!annual)}
          className={`relative h-7 w-12 rounded-full transition-colors ${annual ? "bg-blue-600" : "bg-gray-300"}`}
        >
          <span className={`absolute top-0.5 left-0.5 h-6 w-6 rounded-full bg-white shadow transition-transform ${annual ? "translate-x-5" : ""}`} />
        </button>
        <span className={`text-sm font-medium font-body ${annual ? "text-gray-900 dark:text-white" : "text-gray-400"}`}>
          Annual <Badge variant="emerald">Save 2 months</Badge>
        </span>
      </div>
        ...
        {tiers.map((tier, i) => (
          <FadeIn key={tier.name} delay={i * 0.1}>
            <div className={`relative rounded-2xl border p-8 transition-all duration-300 ${
              ...
              <h3 className="text-xl font-heading font-bold text-gray-900 dark:text-white">{tier.name}</h3>
              <p className="mt-1 text-sm text-gray-400 font-body">{tier.description}</p>
              <div className="mt-6">
              ...
              <p className="mt-1 text-xs text-gray-400 font-mono">{tier.pricePerLead}/lead</p>
              <div className="mt-2 text-sm font-body text-gray-500">
                {tier.leads} &middot; {tier.seats}
              </div>
              {tier.name === "Command" ? (
                <CalBookingButton variant={tier.popular ? "primary" : "secondary"} className="mt-6 w-full">
                  Talk to Sales
                </CalBookingButton>
              ) : (
                <Button href="/signup" variant={tier.popular ? "primary" : "secondary"} className="mt-6 w-full">
                  {tier.cta}
                </Button>
              )}
              <ul className="mt-8 space-y-3">
                {(preview ? tier.features.slice(0, 4) : tier.features).map((f) => (
